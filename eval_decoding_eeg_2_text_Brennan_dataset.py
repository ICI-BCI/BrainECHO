...
def eval_model(dataloaders, device, tokenizer, model, output_all_results_path="./results/temp.txt"):
    ...
    if is_teacher_forcing:
        ...
        with open(output_all_results_path, "w") as f:
            for rawEEG, input_masks, target_ids, target_mask, sent_ids, target_string in tqdm(
                dataloaders["test"], ncols=80
            ):
                # load in batch
                rawEEG_batch = rawEEG[:, :, :2400].to(device).float()
                # split mel spectrogram into segments
                if split_mel:
                    rawEEG_batch = rearrange(rawEEG_batch, "b c (p t)->(b p) c t", t=800)
                if input_noise:
                ...
                logits = output.logits  # b*l*50265

                probs = logits.softmax(dim=-1)
                values, predictions = probs.topk(1)  # 从 词库表中挑选一个概率最大的词作为输出
                ...
                if random_select:
                    all_sent_ids = set(range(140))
                    # all_sent_ids = set(range(140)) - {7, 9, 19, 23, 42, 47, 58, 65, 76, 77, 79, 102, 128, 132}
                    all_sent_ids = list(all_sent_ids)
                    random_ids = random.choices(all_sent_ids, k=rawEEG_batch.shape[0])
                    predicted_string = all_sents[random_ids]
...
if __name__ == "__main__":
    ...
    with open(dataset_path + "../log_mel_features.pickle", "rb") as f:
        log_mel_features = pickle.load(f).to(device)  # (84, 80, 2400)
    with open(dataset_path + "../all_sents.pickle", "rb") as f:
        all_sents = pickle.load(f)

    """ dataset """
    test_set = EEG_dataset(path=dataset_path + "test", subject=subject)
