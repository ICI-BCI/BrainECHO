...
from dataset import EEG_dataset_whisper as EEG_dataset
...
def train_model(
    ...
    for epoch in range(num_epochs):
        ...
        for phase in phases:
            ...
            # Iterate over data.
            for rawEEG, input_masks, target_ids, target_mask, sent_ids, target_string in tqdm(dataloaders[phase], ncols=80):
                b = rawEEG.shape[0]
                rawEEG_batch = rawEEG[..., :2400].to(device).float()
                ...
                if model.task == "task1":
                    ...
                    target_mel_feature_batch = log_mel_features.index_select(0, sent_ids).to(device)
                    if split_mel:
                    ...
                    target_ids_batch[target_mask_batch == 0] = -100
                ...
                # forward
                if phase == "train":
                    out = model(rawEEG_batch, target_ids_batch, target_mel_feature_batch, loss=latent_loss)
                else:
                    with torch.no_grad():
                        out = model(rawEEG_batch, target_ids_batch, target_mel_feature_batch, loss=latent_loss)
                loss = out.loss
                if model.task == "task1":
                    mel_loss = out.mel_loss  # mel reconstruction loss
                    del target_mel_feature_batch
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="EEG-to-Text")
    ...
    init_logger(args)
    logger = getLogger()
    logger.info("dataset_path:{}".format(dataset_path))
    ...
    """ dataset """
    train_set = EEG_dataset(path=dataset_path + "train", subject=subject)
    valid_set = EEG_dataset(path=dataset_path + "valid", subject=subject)
    ...
    valid_dataloader = DataLoader(valid_set, batch_size=batch_size, shuffle=False, num_workers=4, pin_memory=True)
    ...
    """ prepare log mel """
    with open(dataset_path + "../log_mel_features.pickle", "rb") as f:
        log_mel_features = pickle.load(f)  # (140, 80, 1200)
    ...
    del noise_mel
    print("load best model for alignment")
    model.load_state_dict(torch.load(checkpoint_path_best), strict=True)
