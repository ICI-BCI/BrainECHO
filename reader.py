...
# modified from https://github.com/NeuSpeech/NeuSpeech1/blob/main/utils/reader.py
...
class MEGDataset(Dataset):
    def __init__(
        ...
        """
        super(MEGDataset, self).__init__()
    ...
    def _get_list_data(self, idx):
        ...
        assert eeg_file is not None
        dataset_name = None
        if "schoffelen" in eeg_file:
            dataset_name = "schoffelen"
        elif "meg-masc-main" in eeg_file:
        ...
        if self.modal == "eeg":
            ...
            sample, sample_rate = soundfile.read(eeg_file, dtype="float32", always_2d=True)  # [len,ch]
        ...
        # 重采样
        audio, audio_sample_rate = soundfile.read(audio_file, dtype="float32")

        return sample, sample_rate, transcript, language, audio
    ...
    def _get_list_data_random(self, idx):
        ...
        if np.random.rand() > 0.5:
            ...
            for i in range(3):  # 最多加三句
                ...
                new_sample, sample_rate, new_transcript, language = self._get_list_data(
    ...
    def shift_data_transcript(self, sample, transcript):
        ...
        assert isinstance(transcript, list), f"transcript应该为list，当前为：{type(transcript)}"
        length = max(
            int((transcript[-1]["end"]) * self.signal_sample_rate), sample.shape[1]
        )  # notice 应该设置到最长的这个长度，来避免移出去。
        assert (
            length / self.signal_sample_rate < self.max_duration
        ), f"做数据漂移的时间长度必须小于{self.max_duration}s,现在的长度是{length}，采样率是{self.signal_sample_rate},时间长度是{length / self.signal_sample_rate}"
        # try:
    ...
    def __getitem__(self, idx):
        ...
        if self.combine_sentences:
            ...
            sample, sample_rate, transcript, language, audio = self._get_list_data(idx=idx)
            ...
            if self.timestamps:
                ...
                if self.modal == "speech":
                    data["input_features"] = self.processor(
                        audio=sample, sampling_rate=self.signal_sample_rate
                    ).input_features
    ...
    def padding_sample(self, sample):
        ...
        sample = np.pad(sample, pad_width=((0, 0), (0, max_length - sample.shape[-1])))  # 零填充至统一长度(208, 200*30)
        # print(f'sample.shape:{sample.shape}')
        assert sample.shape == (
            self.modal_ch,
            int(self.signal_sample_rate * self.max_duration),
        ), " sample shape should be [self.modal_ch,int(self.signal_sample_rate*30))]"
    ...
    def pad_sample_ch(self, sample):
        ...
        assert (
            sample.shape[0] == self.modal_ch
        ), f"after pad ch, sample channel should be {self.modal_ch}, now is {sample.shape[0]}"
        return sample
    ...
    def augment_audio(self, sample, sample_rate):
        for k, v in self.augment_configs.items():
            ...
            if k == "noise" and torch.rand(1).item() < v["prob"]:
                if self.modal == "eeg":
                    sample = add_gaussian_noise(sample, snr_range=(v["min_snr_dB"], v["max_snr_dB"]))
    ...
    def add_noise(self, sample, sample_rate, noise_path, snr_dB, max_gain_db=300.0):
        ...
        noise_sample = noise_sample * 10.0 ** (noise_gain_db / 20.0)
        # 固定噪声长度
...
def meg_collate_fn(batch):
    ...
    collated_batch = {
        "input_features": torch.stack(
            [torch.tensor(feature["input_features"][0], dtype=torch.float32) for feature in batch]
        )
    }
    ...
    audio_batch = [feature["audio"] for feature in batch]
    log_mel_features = processor(audio_batch, sampling_rate=16000, return_tensors="pt").input_features
    collated_batch["log_mel_features"] = log_mel_features[..., :2400]
